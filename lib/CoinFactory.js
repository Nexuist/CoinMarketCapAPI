...
module.exports = class CoinFactory {
  ...
  produce(tr) {
    ...
    var coin = {
      ...
      "supply": td(5).replace(/\D/g, "").trim() / 1.0, // td(4).replace(/\D/g, "").trim(), // Remove all non-numeric characters
      "volume": self.currencyDictionary(tds.eq(4).find("a").eq(0)), // self.currencyDictionary(tds.eq(5).find("a").eq(0)),
      "change": td(6).replace("%", "").trim() / 1.0,
      "timestamp": (Date.now() / 1000).toString() / 1.0
    }
    return coin
  }
}
