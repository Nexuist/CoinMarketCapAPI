    ...
    email(coin) {
        ...
        fs.readFile('emails/alert.html', 'utf8', function(err, data) {
            ...
            function formatBTC(btc) {
                if (!btc) {
                    return 0.00;
                } else if (typeof btc === 'string' || btc instanceof String) {
                    try {
                        var btcFloat = parseFloat(btc);
                        if (isNaN(btcFloat)) {
                            return 0.00;
                        } else {
                            return btcFloat.toFixed(15).replace(/0+$/, "");
                        }
                    } catch (error) {
                        return 0.00;
                    }
            ...
            html = html.replace("{{ symbol }}", coin.symbol.toUpperCase());
            html = html.replace("{{ price_usd }}", currencyFormatter.format(coin.price.usd, { precision: 2, code: 'USD' }));
            html = html.replace("{{ price_btc }}", formatBTC(coin.price.btc));
            html = html.replace("{{ volume_usd }}", currencyFormatter.format(coin.volume.usd, { precision: 0, code: 'USD' }));
            html = html.replace("{{ volume_btc }}", formatBTC(coin.volume.btc));
            html = html.replace("{{ link }}", coin.link);
                ...
                rows.each(function(i) {
                    ...
                    CoinDatabase.checkIfCoinExist(newCoin, function(err, response) {
                        ...
                        if (response == 0) {
                            ...
                            var price_threshold = parseFloat(process.env.PRICE_THRESHOLD);
                            var volume_threshold = parseFloat(process.env.VOLUME_THRESHOLD);
                            
                            // Check  Price and Volume threshold
                            
                            CoinDatabase.getCoin(newCoin.symbol, function (err, response) {

                              if (newCoin.price.usd && (newCoin.price.usd*price_threshold >= response.price.usd)){
                                self.email(newCoin);
                              }
                              if (newCoin.volume.usd && (newCoin.volume.usd*volume_threshold >= response.volume.usd)) {
                                self.email(newCoin);
                              }
